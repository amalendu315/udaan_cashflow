...
//             INSERT (date, projected_inflow, actual_inflow_id, closing, created_at, updated_at)
//             VALUES (source.date, source.projected_inflow, @actual_inflow_id, @closing, GETDATE(), GETDATE());
//         `);
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
      ...
      await transaction
        ...
        .input("actual_inflow_id", sql.Int, actualInflowId).query(`
          ...
          WHEN NOT MATCHED THEN
            ...
            VALUES (
              source.date, 
              source.projected_inflow, 
              @actual_inflow_id, 
              NULL,  -- Closing will be calculated dynamically
              GETDATE(), 
              GETDATE()
            );
        `);
    ...
    // 🔹 Execute `sp_update_closing` after processing all rows
    await transaction.request().execute("sp_update_closing");
